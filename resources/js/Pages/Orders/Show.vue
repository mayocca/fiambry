<template>
  ...
  <n-page-header class="m-4" @back="router.visit('/')">
    <template #title>
      <span class="text-2xl font-light">Pedido de {{ order.user.name }}</span>
    </template>
  </n-page-header>
  ...
  <div class="p-4 flex flex-col sm:flex-row gap-2">
    <n-card title="Sumario de productos">
      <Summary class="mb-2">
        <SummaryItem v-for="(product, index) in products_summary" :key="index" :product="product" />
      </Summary>
      <template #action>
        <n-space justify="end">
          <n-button
    ...
    <n-card title="Detalles">
      ...
      <template v-if="order.user_id === $page.props.auth.user?.id" #action>
        <n-space justify="end">
          <n-button type="info" @click="$inertia.visit(`/orders/${order.id}/edit`)">
            Editar detalles
          </n-button>
          <n-button type="primary" @click="$inertia.visit('/orders')">Finalizar</n-button>
        </n-space>
      </template>
    </n-card>
  </div>
...
import { NButton, NCard, NPageHeader, NSpace } from "naive-ui";
